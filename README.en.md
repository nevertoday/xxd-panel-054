...
These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 054's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.
...
SELECTIVE MEMORY · MAIN VISUAL · SIX STICKERS · MATTE PRINT · AIRY BLUE
...
**Panel 054** is suited to work that must retain the source's essential content while pursuing this visual direction: **Split one memory into a main image and six unequal fragments**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 054 on this image and recommend the best layout and size first.
...
Aesthetic constraints and rejection rules live only in the [original source brief](references/original-prompt/zh-CN.md); the Skill and runtime adapter handle delivery variables only. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-054-prompt.en.md)
...
/xxd-panel-054 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-054 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-054 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-054 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 054 to a whole directory; shared settings resolve once
/xxd-panel-054 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-054-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-054-prompt.zh-CN.md)
